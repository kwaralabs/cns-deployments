...
###############################################################################
#
#  ERPNext v16 + HRMS — Production Setup via Frappe Docker
#
...
#
#    8. Apps beyond erpnext (e.g. hrms) are baked into the image via
#       APPS_JSON, then installed on the site with bench install-app
#       AFTER bench new-site --install-app erpnext.
#
#  USAGE:
...
# Apps baked into the Docker image.
# All apps listed here are compiled into the image at build time.
# erpnext is installed during bench new-site (--install-app erpnext).
# Additional apps are installed afterward via bench install-app.
APPS_JSON='[
  {
...
log "Building custom image with apps: erpnext, hrms (10-30 minutes)..."
...
# Install additional apps beyond erpnext
...
docker compose --project-name "${PROJECT_NAME}" -f "$YAML_FILE" \
  exec -T backend \
  bench --site "${SITE_DOMAIN}" list-apps
...
echo ""
echo "  # Install another app (must be in the image)"
echo "  docker compose --project-name ${PROJECT_NAME} -f ~/gitops/${PROJECT_NAME}.yaml exec backend \\"
echo "    bench --site ${SITE_DOMAIN} install-app APP_NAME"
echo ""