...
###############################################################################
#
#  ERPNext v16 + Frappe LMS — Production Setup via Frappe Docker
#
...
#      └── Single Bench (ERPNext + Frappe LMS baked into one custom image)
#
...
#
#    8. Both erpnext and lms are baked into the same image via APPS_JSON.
#       Both apps are installed on the site via --install-app flags.
#       Frappe LMS targets the "main" branch (no versioned release branch).
#
#  USAGE:
#    1. Point DNS A record to your server IP
#    2. Edit the CONFIGURATION section below
#    3. Run as root on a fresh Ubuntu 24.04 VPS:
#         chmod +x setup-erpnext-lms.sh
#         ./setup-erpnext-lms.sh
...
# Both ERPNext and LMS are baked into the same image.
# Frappe LMS uses the "main" branch — it has no versioned release branch.
APPS_JSON='[
  {
...
[ "$(id -u)" -ne 0 ] && err "Run as root: sudo ./setup-erpnext-lms.sh"
...
cat > "$PASS_FILE" << EOF
# ERPNext + Frappe LMS Credentials — $(date)
...
log "Building custom image with ERPNext + Frappe LMS (10-30 minutes)..."
...
# Verify both apps are present in the built image
log "Verifying apps in image..."
docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" \
  bash -c "ls /home/frappe/frappe-bench/apps/" | grep -q "erpnext" \
  || err "erpnext not found in image"
docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" \
  bash -c "ls /home/frappe/frappe-bench/apps/" | grep -q "lms" \
  || err "lms not found in image"
log "Verified: erpnext and lms both present in image."
...
###############################################################################
step "PHASE 8: Deploy bench (ERPNext + Frappe LMS)"
###############################################################################
...
###############################################################################
step "PHASE 9: Create site and install apps"
###############################################################################
...
log "Site created with ERPNext and Frappe LMS."
...
echo "--- Installed Apps ---"
docker compose --project-name "${PROJECT_NAME}" -f "$YAML_FILE" \
  exec -T backend bench --site "${SITE_DOMAIN}" list-apps 2>/dev/null || true
...
echo ""
echo "  Apps installed:"
echo "    • ERPNext    → https://${SITE_DOMAIN}"
echo "    • Frappe LMS → https://${SITE_DOMAIN}/lms"
echo ""
...
echo "  docker compose --project-name ${PROJECT_NAME} -f ~/gitops/${PROJECT_NAME}.yaml exec backend bench --site ${SITE_DOMAIN} list-apps"
echo ""